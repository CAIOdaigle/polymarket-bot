"""Technical analysis — 7-indicator weighted composite for BTC 5-min direction.

Merged from existing src/btc_sniper/ta_strategy.py (proven in production)
and downloaded signals/technical.py (additional indicators).

Positive score = UP, Negative = DOWN. Magnitude = confidence.
"""
...
from crypto_sniper.feeds.binance_feed import Candle
...
def analyze(
    ...
    Args:
        ...
        tick_prices: (timestamp, price) tuples from WebSocket stream.
    ...
    if tick_prices and len(tick_prices) >= 5:
        ...
        if total_moves > 0:
            ...
            tick_move_pct = (
                (tick_prices[-1][1] - tick_prices[0][1]) / tick_prices[0][1] * 100
            )
